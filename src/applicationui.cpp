...
void ApplicationUI::childCardDone(
	...
	qDebug() << "ChildCardDone data: " << doneMessage.data() << " reason:"
			<< doneMessage.reason();
	if(doneMessage.reason() == "Done"){
		...
		filename += "/";
		filename += doneMessage.data();
		Application::instance()->scene()->setProperty("invokedWithImage",
					filename);
			Application::instance()->scene()->setProperty("invoked",
							true);
	} else {
		// do something
	}
...
void ApplicationUI::onSystemLanguageChanged() {
	...
	QString file_name = QString("ekkesChartInvoker_%1").arg(locale_string);
